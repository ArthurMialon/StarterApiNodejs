...
export default class CRUD extends Route {
  ...
  */
  constructor(config, model) {
    ...
    this.ressource = model;
    this.model     = Database.getModel(this.ressource);
    this.populate  = config.populate || false;
  ...
  addPopulate(query) {
    if (this.populate) {
      this.populate.forEach( model => {
        query = query.populate(model);
      });
    }
  ...
  */
  all(req, res) {
    ...
    let limit = req.query.limit || parseInt(configuration.limit);
    return this.response(res, this.addPopulate(this.model.find().paginate({page: page, limit: limit})));
  }
  ...
  */
  read(req, res) {
    return this.response(res, this.addPopulate(this.model.findOne().where({ id: req.params.id })));
  }

...
export default class Router {
  ...
  constructor(ressource) {
    ...
    methods.forEach((method) => {
      if (ressource.hasMethod(method)) {
        this.initRoutesFor(method, ressource.getRoutes(method), ressource.config.default);
      }
    });

    if(ressource.hasCrud()) {
      this.createCrud(ressource.getCrud(), ressource.getModel(), ressource.config.default);
    }
  ...
  */
  initRoutesFor(method, routes, defaultConfig) {
    let self = this;
    return routes
      .map( route => this.bindDefaultConfig(route, defaultConfig) )
      .map( route => new Route(method, route).toObject() )
      .forEach((route) => {
  ...
  * @param {array} crud configuration
  * @param {string} model name / ressource name
  * @param {mixed} default configuration
  */
  createCrud(crud, model, defaultConfig) {
    let self = this;
    return crud
      .map( endpoint => this.bindDefaultConfig(endpoint, defaultConfig) )
      .map( endpoint => {
        if (model) {
          return new CRUD(endpoint, model).toObject();
        }
        return false;
      })
      .forEach((route) => {
        if(route && !self.hasRoute(route)) {
          self.addRoute(route);
        }
      });
  }
  ...
  */
  bindDefaultConfig(route, defaultConfig) {
    let _defaultConfig = this.clone(defaultConfig);
    if (_defaultConfig) {
      route = merge(_defaultConfig, route);
    }
    return route;
  }

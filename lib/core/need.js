...
module.exports = {
  ...
  check: function(req, route) {
    ...
    for(var n in this.need) {
      ...
      if(is.boolean(this.need[n]))
        if (!this.isBoolean(n, this.need[n])) { this.policies.push(1); }
      // In case of integer
      else if (is.number(this.need[n]))
        if (!this.isNumber(n, this.need[n])) { this.policies.push(1); }
      // In case of String
      else if (is.string(this.need[n]))
        if (!this.isString(n, this.need[n])) { this.policies.push(1); }
    }
  ...
  isString: function(name, value) {
    ...
    if(this.acceptTest.indexOf(parse[0]) != -1) {
      ...
      // In case of a date
        ...
        console.log("Date policies doesn't work yet");
        console.log("That will available soon...");
        // => DATE
  ...
  */
  validateNumber: function(name, value, operator) {
    return ((operator == ">"  && (this.user[name] && this.user[name] < value) ) || (operator == "=" && (this.user[name] && this.user[name] != value) ) || (operator == "<" && (this.user[name] && this.user[name] > value) )) ? false : true;
  },
  ...
  */
  valideString: function(name, value, operator) {
    return ((operator == ">"  && (this.user[name] && this.user[name].length < value)) || (operator == "=" && (this.user[name] &&  this.user[name].length != value)) || (operator == "<" && (this.user[name] && this.user[name].length > value))) ? false : true;
  }
